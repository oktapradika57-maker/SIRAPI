...
def fetch_spreadsheet_data(spreadsheet_id):
    ...
    try: um_rows = ss.worksheet(SHEET_UM).get_all_values()
    except: um_rows = []
    return req_rows, pjb_rows, um_rows
...
cek_nop = st.sidebar.selectbox("📂 NOP", ["-- Pilih NOP --"] + list(MASTER_DATA.keys()), key="cek_nop")

if cek_nop != "-- Pilih NOP --":
    cek_nama = st.sidebar.selectbox("👤 Nama Anda", ["-- Pilih Nama --"] + MASTER_DATA[cek_nop]["names"], key="cek_nama")
    if st.sidebar.button("Cari Histori Saya"):
        if cek_nama != "-- Pilih Nama --":
            req_r, pjb_r, _ = fetch_spreadsheet_data(MASTER_DATA[cek_nop]["spreadsheet_id"])
            ...
            else: st.sidebar.info("Belum ada data pengajuan untuk nama ini.")
...
# ==========================================
    ...
    if nop_cari != "-- Pilih NOP --":
        ...
        for r in req_r[1:]:
            if len(r) > 5:
                tkt = r[3].strip().upper()
                if tkt not in pjb_tickets_all and tkt != "":
                    pending_list.append({"Tanggal Req": r[1], "Nama Petugas": r[5], "No Tiket": r[3], "Keperluan": r[8] if len(r)>8 else ""})
        
        st.markdown("<div class='section-title'>📋 Daftar Tiket Tertunda (Belum PJB)</div>", unsafe_allow_html=True)
        if pending_list:
            st.info(f"Terdapat **{len(pending_list)} tiket** yang belum dipertanggungjawabkan pada area {nop_cari}:")
            st.dataframe(pd.DataFrame(pending_list), hide_index=True, use_container_width=True)
        else:
            st.success(f"Luar biasa! Seluruh tiket di area {nop_cari} sudah selesai di-PJB-kan.")
        
        st.markdown("<div class='section-title'>🔍 2. Pencarian Data Tiket</div>", unsafe_allow_html=True)
...
# ==========================================
    ...
    if nop_admin != "-- Pilih NOP --" and pass_admin == ADMIN_PASSWORDS[nop_admin]:
        ...
        with st.form("form_tambah_um"):
            st.info("Input di bawah ini akan tersimpan permanen ke dalam tabel 'Data UM' sebagai histori Kas Masuk.")
            col_u1, col_u2, col_u3 = st.columns([1, 2, 1])
            with col_u1: tgl_um = st.date_input("Tanggal UM Masuk")
            with col_u2: um_nobis = st.text_input("Deskripsi / Dasar Dokumen UM (Cth: Nobis Agustus)")
            with col_u3: um_nominal = st.number_input("Nominal UM (Rp)", min_value=0, step=1000)
            
            submit_um = st.form_submit_button("💾 Rekam Uang Muka ke Buku Kas")
            if submit_um:
                if not um_nobis.strip(): st.error("Harap isi deskripsi dokumen UM!")
                elif um_nominal <= 0: st.error("Nominal UM tidak boleh 0!")
                else:
                    waktu_input = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
                    append_data(SHEET_UM, [waktu_input, tgl_um.strftime("%d/%m/%Y"), um_nobis, um_nominal], target_ss)
                    st.success(f"✅ Uang Muka sejumlah Rp {um_nominal:,.0f} berhasil dicatat!")
                    time.sleep(1)
                    st.rerun()
        ...
        if st.button("🔄 Kalkulasi Neraca", type="primary", use_container_width=True):
            with st.spinner("Mensinkronisasi buku besar..."):
                ...
                if len(um_rows) > 1:
                    ...
                    mask_um = (df_um['Tanggal'].dt.date >= start_date) & (df_um['Tanggal'].dt.date <= end_date)
                    df_um_filtered = df_um.loc[mask_um].copy()
                ...
                m3.metric("🧾 Sisa Kas Area", f"Rp {sisa_saldo:,.0f}", delta=sisa_saldo, delta_color="normal")
                ...
                with col_tab1:
                    ...
                    if total_um_masuk > 0:
                        show_um = df_um_filtered[['Tanggal', 'Deskripsi', 'Nominal']].copy()
                ...
                with col_tab2:
                    ...
                    if total_pengeluaran > 0:
                        show_pjb = df_pjb_filtered[['Tanggal PJB', 'Tiket', 'Nama', 'Nominal PJB']].copy()
